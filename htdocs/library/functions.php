...
function displayBorrowedTransaction($value) {
	$created = date( 'm/d/y', strtotime($value['date_created']));
	$return = $value['return_date'];
	if ($return == '0000-00-00') {
		$return = "Not Specified";
	} else {
		$return = date( 'm/d/y', strtotime($value['return_date']));
	}
...
function displayLentTransaction($value) {
	...
	echo "</div></div>";
	if ($value['active'] != "NO") {
		echo "<div class='returned'><a class='button long-button' href='?name=ct&value=$value[transaction_id]'>Returned</a></div>";
		// echo "<div class='returned'><button class='button long-button' value='?name=ct&value=$value[transaction_id]'>Returned</button></div>";
	}
	echo "</div>";
}
